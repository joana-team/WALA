...
public class SSABuilder extends AbstractIntStackMachine {
  ...
  private static class SSA2LocalMap implements com.ibm.wala.ssa.IR.SSA2LocalMap {
    ...
    public Map<Integer, Set<String>> getLocalNames() {
      final Map<Integer, Set<String>> allLocalNames = new HashMap<>();
      final IBytecodeMethod m = shrikeCFG.getMethod();
      try {
        if (!m.hasLocalVariableTable()) {
          ...
          for (int index = 0; index < localStoreMap.length; index++) {
            ...
            if (p != null ) {
              ...
              final BasicBlock block = shrikeCFG.getBlockForInstruction(index);
              final String name;
              if (block.getLastInstructionIndex() == index) {
                assert shrikeCFG.getNormalSuccessors(shrikeCFG.getBlockForInstruction(index)).size() == 1;
                final BasicBlock successor = shrikeCFG.getNormalSuccessors(shrikeCFG.getBlockForInstruction(index)).iterator().next();
          ...
          for (int blockIndex = 0; blockIndex < block2LocalState.length; blockIndex++) {
            ...
            if (localsState != null) {
              final BasicBlock block = shrikeCFG.getNode(blockIndex);
              for (int localNumber = 0; localNumber < localsState.length; localNumber++) {
                ...
                final List<String> names = new ArrayList<String>(1) {
                  private static final long serialVersionUID = 2607892882748705890L;
                ...
                if (block.isEntryBlock()) {
                  ...
                  for (BasicBlock predecessor : shrikeCFG.getNormalPredecessors(block)) {
                  ...
                  for (BasicBlock predecessor : shrikeCFG.getExceptionalPredecessors(block)) {
